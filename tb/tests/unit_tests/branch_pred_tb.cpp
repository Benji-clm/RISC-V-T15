...
// Test that the predictor starts off predicting not taken and transitions
// after multiple taken branches
TEST_F (BranchPredTestbench, BasicPredictionTest)
{
  ...
  EXPECT_EQ (top->branch_targetF, (uint32_t)0x104)
      << "Initially target should be pcF+4";
  ...
  // Apply 4 taken updates to push the state machine to the strongly taken
  // state
  for (int i = 0; i < 4; i++)
    {
      tick ();
  ...
  EXPECT_EQ (top->branch_predictF, 1)
      << "After training, should predict taken.";
  EXPECT_EQ (top->branch_targetF, branch_target)
      << "After training, target should match what we stored.";
